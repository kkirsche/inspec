...
module Compliance
  class ComplianceCLI < Inspec::BaseCLI # rubocop:disable Metrics/ClassLength
    ...
    def automate(server) # rubocop:disable Metrics/AbcSize
      options['server'] = server
      ...
      if url && !options['user'].nil? && !options['ent'].nil?
        if !options['dctoken'].nil? || !options['usertoken'].nil?
          msg = login_automate(url, options['user'], options['dctoken'], options['usertoken'], options['ent'])
        else
          puts "Please specify a token using --dctoken='DATA_COLLECTOR_TOKEN' or usertoken='AUTOMATE_TOKEN' "
          exit 1
        end
      else
        puts "Please login to your automate instance using 'inspec compliance automate SERVER --user AUTOMATE_USER --dctoken DC_TOKEN (or --usertoken USER_TOKEN) --ent AUTOMATE_ENT' "
        exit 1
      end
      puts '', msg
    end
    ...
    def login_automate(url, user, dctoken, usertoken, ent)
      config = Compliance::Configuration.new
      config['server'] = url
      config['ent'] = ent
      config['user'] = user
